...
impl HistoryDataCollector {
    ...
    pub fn add_from_gizmo_results(&mut self, time: Instant, gizmo_results: &HashMap<Uuid, GizmoResult>) -> Result<(), anyhow::Error> {
    ...
    pub fn for_each_data_point<F>(&self, from: Instant, to: Instant, mut f: F)
    where
        F: FnMut(&DataPoint),
    {
        // TODO: binary search
        for (_, data_points) in &self.stored_data {
